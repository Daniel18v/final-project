...
namespace Mb88xx
{
	...
	Processor::Processor(Emulator::Scheduler& scheduler,
						...
						byte* programRom, ushort programRamLength,
						byte* dataRam, byte dataRamLength,
						Emulator::IOHandler& ioHandler,
						byte* pla, byte plaLength,
...
#pragma warning (disable : 4355) //  'this' : used in base member initializer list
		serialTimer("SerialTimerCallback", scheduler,
			[this](INT64 param, INT64 currentTime) { this->SerialTimerCallback(param); }),
#pragma warning (default : 4355)
    ...
    void Processor::SerialTimerCallback(INT64 param)
    {
        status.SBcount++;
        ...
        if (!status.sf)
        {
            status.SB = (byte)((status.SB >> 1) | ((ReadPort(Port::MB88_PORTSI) != 0) ? 8 : 0));
    ...
    {
        status.TL = (byte)((status.TL + 1) & 0x0f);
        if (status.TL == 0)
        {
            status.TH = (byte)((status.TH + 1) & 0x0f);
            if (status.TH == 0)
            {
                status.vf = true;
    ...
    byte Processor::GetPla(int inA, int inB)
    {
        byte index = (byte)(((inB & 1) << 4) | (inA & 0x0f));
        if (null != pla)
        {
	...
	void Processor::UpdatePio(int cycles)
    {
        ...
        {
            status.TP = (byte)(status.TP + cycles);
        ...
        if ((status.interruptPending & status.pio) != 0)
        {
            ...
            status.SP[status.SI] |= (ushort)(status.cf ? 1 << 15 : 0);
            status.SP[status.SI] |= (ushort)(status.zf ? 1 << 14 : 0);
            status.SP[status.SI] |= (ushort)(status.st ? 1 << 13 : 0);
    ...
    void Processor::UpdatePioEnable(byte newPio)
    {
        ...
        if (0 != ((status.pio ^ newPio) & 0x30))
        {
            ...
            {
                serialTimer.Start((INT64)(1e6 / (clockFrequency / SERIAL_PRESCALE)), false);
            }
            else
            {
