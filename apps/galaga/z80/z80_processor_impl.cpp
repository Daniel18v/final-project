...
namespace Z80 {
    ...
    readonly byte Z80ProcessorImpl::cc_op[] =
    {
    ...
    readonly byte Z80ProcessorImpl::cc_cb[] =
    {
    ...
    readonly byte Z80ProcessorImpl::cc_ed[] =
    {
    ...
    readonly byte Z80ProcessorImpl::cc_xy[] =
    {
    ...
    readonly byte Z80ProcessorImpl::cc_xycb[] =
    {
    ...
    readonly byte Z80ProcessorImpl::cc_ex[] =
    {
    ...
    readonly bool Z80ProcessorImpl::parity[] =
    {
    ...
    byte Z80ProcessorImpl::flag_SZHVC_add[256][256][2];
    byte Z80ProcessorImpl::flag_SZHVC_sub[256][256][2];

    byte Z80ProcessorImpl::flag_SZ[256];
    byte Z80ProcessorImpl::flag_SZP[256];
    byte Z80ProcessorImpl::flag_SZ_BIT[256];
    byte Z80ProcessorImpl::flag_SZHV_dec[256];
    byte Z80ProcessorImpl::flag_SZHV_inc[256];

	readonly byte Z80ProcessorImpl::rstMask[] =
    {
	...
	Z80ProcessorImpl::Z80ProcessorImpl(double clockFrequency, byte ram[], Emulator::IOHandler& ioHandler,
...
#pragma warning (default: 4355)
    ...
    void Z80ProcessorImpl::InitFlagArrays()
    {
        for (int i = 0; i < 256; i++)
        {
            ...
            flag_SZ[i] = (byte)((i != 0) ? (i & S_mask) : Z_mask);

            flag_SZ_BIT[i] = (byte)((i != 0) ? (i & S_mask) : (Z_mask | PV_mask));

            flag_SZP[i] = (byte)(flag_SZ[i] | ((0 != (totBits % 2)) ? 0 : PV_mask));
            ...
            flag_SZHV_dec[i] = (byte)(flag_SZ[i] | N_mask);
            if (i == 0x7f)
            {
        ...
        for (int oldval = 0; oldval < 256; oldval++)
        {
            for (int newval = 0; newval < 256; newval++)
            {
                ...
                byte flags = (byte)((0 != newval) ? ((0 != (newval & 0x80)) ? S_mask : 0) : Z_mask);
                ...
                // adc with carry set
                val = newval - oldval - 1;
                flags = (byte)((0 != newval) ? ((0 != (newval & 0x80)) ? S_mask : 0) : Z_mask);
                if ((newval & 0x0f) <= (oldval & 0x0f))
                {
                    flags |= H_mask;
                }
                ...
                val = oldval - newval;
                flags = (byte)(N_mask | ((0 != newval) ? ((0 != (newval & 0x80)) ? S_mask : 0) : Z_mask));
                if ((newval & 0x0f) > (oldval & 0x0f))
                {
                    flags |= H_mask;
                }
                ...
                val = oldval - newval - 1;
                flags = (byte)(N_mask | ((0 != newval) ? ((0 != (newval & 0x80)) ? S_mask : 0) : Z_mask));
                //flags |= (byte)((newval & (Y_mask | X_mask)));	// undocumented flag bits 5+3
                if ((newval & 0x0f) >= (oldval & 0x0f))
                {
                    flags |= H_mask;
                }
    ...
    Z80ProcessorImpl::Instruction Z80ProcessorImpl::FetchDecode(ushort pc)
    {
        ushort pcInit = pc;
	...
	INT64 Z80ProcessorImpl::get_ExecutionTimeInCurrentTimeslice() const
	{
    ...
    INT64 Z80ProcessorImpl::Execute(INT64 us) // microseconds
    {
		aborted = false;
        ...
        do
        {
            ...
            FetchDecode(this->PC);
            this->PC += (ushort)(this->instr.length);
            this->R++;
	...
	void Z80ProcessorImpl::TakeInterrupt()
    {
        ...
        switch (this->interruptMode)
        {
			case InterruptMode::Mode0:
                ...
                // of these were found we assume a 1 byte opcode was placed on the databus.
                if (this->irqVector != 0x00) // check for nop
                {
                    switch (this->irqVector & 0xff0000)
                    {
                        case 0xcd0000:	// call 
                            Push_pc();
                            this->PC = (ushort)(this->irqVector & 0xffff);
                        ...
                        default:		// rst (or other opcodes?) 
                            Push_pc();
                            this->PC = (ushort)(this->irqVector & 0x0038);
			...
			case InterruptMode::Mode2:
                ...
                this->irqVector = ((uint)(this->irqVector & 0xff)) | ((uint)(this->I << 8));
                Push_pc();
                this->PC = ReadMemory16((ushort)this->irqVector);
