...
namespace Galaga {
	...
	MainCpu::MainCpu(GalagaMachine& galaga, byte* ram, uint ramLength, Emulator::IOHandler& ioHandler) :
#pragma warning (disable: 4355) // 'this' : used in base member initializer list
        Inherited(galaga.scheduler, 0,
			new Z80::Processor(galaga.ClockFrequency, ram, ioHandler, "main", this, *this)),
#pragma warning (default: 4355)
    ...
    bool MainCpu::WriteMemory(ushort address, byte value)
	{
        if (address < 0x4000)
        {
            // rom - do nothing
    ...
    bool MainCpu::ReadMemory(ushort address, byte& value)
	{
        if (address < 0x4000)
        {
            value = rom[address];
