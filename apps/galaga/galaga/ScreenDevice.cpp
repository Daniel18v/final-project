...
namespace Galaga {
	...
	ScreenDevice::ScreenDevice(GalagaMachine& galaga, byte* ram) :
		galaga(galaga),
		ram(ram),
        romTxt(),
        romSprites(),
		...
		vblankBeginTimer(null)
	{
		...
		vblankBeginTimer = new Emulator::CTimer("OnVblankBegin", galaga.scheduler,
			[this](INT64 param, INT64 currentTime) { OnVblankBegin(param); }),
		Reset();
	}
...
//#endif // #ifndef _NOFILEAPI
        ...
        {
            byte planebit = (byte)(1 << (planes - 1 - plane));
	...
	std::unique_ptr<Sprite> ScreenDevice::GenerateSprite(SpriteData spriteData) const
    {
        int offs[2][2] =
        {
            { 0, 1 },
        ...
        int yOffset[] = { 0x00, 0x01, 0x02, 0x03, 0x40, 0x41, 0x42, 0x43, 0x80, 0x81, 0x82, 0x83, 0xc0, 0xc1, 0xc2, 0xc3 };
        int xOffset[] = { 0x00, 0x08, 0x10, 0x18, 0x20, 0x28, 0x30, 0x38, 0x100, 0x108, 0x110, 0x118, 0x120, 0x128, 0x130, 0x138 };
        int pOffset[2] = { 0, 4 };
        const int planes = 2;
        ...
        for (int i = 0; i < (spriteData.sizey / 16); i++)
        {
            for (int j = 0; j < (spriteData.sizex / 16); j++)
            {
                ...
                int startx = spriteData.flipx ? (((spriteData.sizex / 16) - 1) - j) : j;
                int starty = spriteData.flipy ? (((spriteData.sizey / 16) - 1) - i) : i;
                ...
                if (spriteData.flipx)
                {
                    index += 15;
                    incx = -1;
                    incy = spriteData.flipy ? (-spriteData.sizex + 16) : (spriteData.sizex + 16);
                }
                ...
                for (int y = 0; y < 16; y++)
                {
                    for (int x = 0; x < 16; x++)
                    {
                        int iCt = dp[x * 16 + y] + 4 * spriteData.color; // code review! (rotated?)
    ...
    void ScreenDevice::DrawSprites(Frame& frame)
    {
        for (int index = 0; index < TotSprites; index++)
        {
            ...
            int left = ram[VideoRamBase2 + 0x380 + 2 * index + 1] - 40 + 0x100 * (ram[VideoRamBase3 + 0x380 + 2 * index + 1] & 0x03);
            int top = 256 - ram[VideoRamBase2 + 0x380 + 2 * index] + 1;	// sprites are buffered and delayed by one scanline
            spriteData.flipx = 0 != (ram[VideoRamBase3 + 0x380 + 2 * index] & 0x01);
            spriteData.flipy = 0 != ((ram[VideoRamBase3 + 0x380 + 2 * index] & 0x02) >> 1);
            spriteData.sizex = (byte)(16 * (1 + ((ram[VideoRamBase3 + 0x380 + 2 * index] & 0x04) >> 2)));
            spriteData.sizey = (byte)(16 * (1 + ((ram[VideoRamBase3 + 0x380 + 2 * index] & 0x08) >> 3)));

            top -= spriteData.sizey - 16;
            top = (top & 0xff) - 32;	// fix wraparound

            if (null == sprites[index] || spritesData[index] != spriteData)
            {
    ...
    void ScreenDevice::DrawStars(Frame& frame)
    {
        starfield->DrawStars(frame);
    }
	...
	void ScreenDevice::InitMappings()
    {
        ...
        for (int row = 0; row < Rows; row++)
        {
            for (int col = 0; col < Columns; col++)
            {
                ...
                maxMemoryIndex = MAX(maxMemoryIndex, memIndex);
            }
        }
        maxMemoryIndex++;

		ASSERT(null == memoryToLogical);
        memoryToLogical = new int[maxMemoryIndex];

		ASSERT(null == logicalToMemory);
        logicalToMemory = new int[maxLogicalIndex];

		ASSERT(null == dirtyMap);
    ...
    bool ScreenDevice::WriteMemory(ushort address, byte value)
    {
        if (address >= VideoRamBase0 && address < VideoRamBase0 + VideoRamLength0)
        {
            ushort offset = (ushort)(address - VideoRamBase0);
            ram[address] = value;
    ...
    void ScreenDevice::OnVblankBegin(INT64 param)
    {
    ...
    INT64 ScreenDevice::TimeUntilScanline(int scanline) const
    {
        INT64 vpos = (scanline + ScreenTotalHeight - VBlank0Line) % ScreenTotalHeight;
        INT64 targetDelta = vpos * ScanTime; // us
        INT64 currentDelta = galaga.scheduler.get_CurrentTime() - vblankStartTime; // us
    ...
    INT64 ScreenDevice::get_TimeUntilVblankStart() const
    {
